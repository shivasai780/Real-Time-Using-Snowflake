...
update customer set FIRST_NAME='Jessica', update_timestamp = current_timestamp()::timestamp_ntz where customer_id=72;--updating the customer first name 
delete from customer where customer_id =73 ;--deleting the row from the customer

select * from customer_history where customer_id in (72,73,223136);--we still didnt keep anything in the Customer History
select * from customer_table_changes;
